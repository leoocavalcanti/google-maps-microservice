...
export function NewRoutePage() {
  ...
  async function searchPlaces(event: FormEvent) {
    ...
    const [sourceResponse, destinationResponse] = await Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_API_PROXY_URL}/places?text=${source}`),
      fetch(`${process.env.NEXT_PUBLIC_API_PROXY_URL}/places?text=${destination}`),
    ]);
    ...
    const directionsResponse = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/directions?originId=${placeSourceId}&destinationId=${placeDestinationId}`
    );
  ...
  async function createRoute() {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URLL}/routes`, {
      method: "POST",
      headers: {
